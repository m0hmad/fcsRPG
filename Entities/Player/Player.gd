...
#Player movement speed (pixel/sec)
export var speed = 75
...
func _ready():
	...
	# TranzitZone
	stat_tranzit_update(Global.stat_palyer)
...
func _physics_process(delta):
	...
	# Animaate player based on direction
...
func animates_player(direction: Vector2):
	if direction != Vector2.ZERO:
		...
		last_direction = 0.5 * direction + 0.5 * direction
		
		# Choose walk animation based on movemetn direction
		var animation = get_animation_direction(last_direction) + "_walk"
		
		# Play the walk animation
		$Sprite.play(animation)
	else:
		# Choosw idle animation based on last movement direction and play it
		var animation = get_animation_direction(last_direction) + "_idle"
		$Sprite.play(animation)
		
		# Animation FPS based on player speed (for joypad with analogue sticks)
