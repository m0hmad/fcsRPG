...
# Spawner variables
export var spawn_area : Rect2 = Rect2(50, 100, 320, 180)
export var max_skeletons = 15
export var start_seketons = 5
...
func _ready():
	...
	if not get_parent().load_saved_game:
		for i in range(start_seketons):
			instance_skeleton()
		skeleton_count = start_seketons
...
func test_position(position : Vector2):
	...
	# Check if there's a tree in this posiion
